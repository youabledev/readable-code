...
public class MinesweeperGame {
    ...
    public static final int LAND_MINE_COUNT = 10; // 지뢰의 갯수
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try {
                ...
                if (doesUserWinTheGame()) { // 구체에 가까운 내용
    ...
    private static void actOneCell(String cellInput, String userActionInput) {
        int selectedColIndex = getSelectedColIndex(cellInput);
        int selectedRowIndex = getSelectedRowIndex(cellInput);

        if (doesUserChooseToPlantFlag(userActionInput)) { // userActionInput이 2인지 확인하는 구체적인 내용
    ...
    private static int convertRowFrom(char cellInputRow) {
        ...
        if (rowIndex >= BOARD_ROW_SIZE) {
            throw new AppException("row 잘못 입력된 값입니다.");
        }
        return rowIndex;
    }
    ...
    private static int convertColFrom(char cellInputCol) {
        switch (cellInputCol) {
            ...
            case 'j':
                return 9;
            default:
                throw new AppException("col 잘못된 입력입니다.");
        }
    }
    ...
    private static int countNearbyLandMinds(int row, int col) {
        int count = 0;
